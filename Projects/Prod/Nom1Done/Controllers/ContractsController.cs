    ...
    public class ContractsController : BaseController
    {
        ...
        public ActionResult Index(int pipelineId)
        {
            ...
            ContractListDTO _contractListModel = new ContractListDTO();
             ...
             var pipelineData = _IPipelineService.GetPipeline(pipelineId);           
            _contractListModel.PipelineId = pipelineId;
            ...
            _contractListModel.ContractList= _IContractService.GetContracts(_contractListModel.PipelineId, companyID).ToList();
            _contractListModel.PipelineDetails = pipelineData.Name;
        ...
        public ActionResult Create(int pipelineId)
        {
       ...
       // [ValidateAntiForgeryToken]
        ...
        /// <returns></returns>
        public bool OnflyContractSave(string ContractNumber, string FuelPercentage,int PipelineId)
        {
            var identity = (ClaimsPrincipal)Thread.CurrentPrincipal;
            string company = identity.Claims.Where(c => c.Type == "CompanyId")
                               .Select(c => c.Value).SingleOrDefault();
        ...
        public ActionResult Edit(int pipelineId, int? id)
        {
            if (id == null)
            {
                return new HttpStatusCodeResult(HttpStatusCode.BadRequest);
        ...
        public ActionResult Delete(int pipelineId, int? id)
        {
            if (id == null)
            {
                return new HttpStatusCodeResult(HttpStatusCode.BadRequest);
            ...
            if (model == null)
            {
                ...
                if (isContractUsed)
                {
                    ...
                    return RedirectToAction("Index", new { pipelineId });
                }
                else {
                    return View(model);
                }
        ...
        public PartialViewResult NotimationsPartials(string popUpFor, int PipelineID)
        {
            var identity = (ClaimsPrincipal)Thread.CurrentPrincipal;
            string company = identity.Claims.Where(c => c.Type == "CompanyId")
                               .Select(c => c.Value).SingleOrDefault();
            ...
            locationList = _ILocationService.GetLocations(PipelineID).ToList();
            if (popUpFor == "RecLoc")
            {
