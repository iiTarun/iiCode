...
namespace Nom1Done.Controllers
{
    ...
    public class UploadNominationsController : BaseController
    {
        ...
        public void DownloadFile(int FileID, int pipelineId)
        {
            UploadedFile file;
            try
            {
        ...
        [HttpPost]
        public ActionResult GetFiles(int pipelineId)
        {
            ...
            try
            {
                ...
                var UserId = identity.Claims.Where(c => c.Type == "UserId")
                               .Select(c => c.Value).SingleOrDefault();
                IEnumerable<UploadFileDTO> uploadedFilesList = uploadNominationService.GetUploadedFiles(pipelineId, UserId);
        ...
        public ActionResult UploadFiles(IEnumerable<HttpPostedFileBase> files, int Id)
        {
            ...
            try
            {
                ...
                var PipelinesForUser = pipelineService.GetAllPipelineList(GetCurrentCompanyID(), UserID);
                foreach (var file in files)
                {
                    ...
                    if (file.ContentLength > 0)
                    {
                        ...
                        if (fileExtension == ".xls" || fileExtension == ".xlsx")
                        {
                            ...
                            string query4 = string.Format("Select * from [{0}]", "NonPathed$A2:V");
                            try {                            
                            ...
                            try
                            {
                                ...
                                foreach (DataRow row in batchds.Tables[0].Rows)
                                {
                                    ...
                                    batch.FlowStartDate = bDate;
                                    batch.CycleId = cycleId;
                                    batch.Description = "Bulk Upload - NonPathed Batch";
                                    batch.NomSubCycle = "";
                                    batch.StatusID = (int)statusBatch.Draft;
                                    batch.SubmitDate = DateTime.MaxValue;
                                    batch.ScheduleDate = DateTime.MaxValue;
                                    ...
                                    foreach (DataRow nomRow in nonpahedds.Tables[0].Rows) {

                                        var PipeDunsRec = Convert.ToString(nomRow["Duns No"]);
                                        var PipeDunsDel = Convert.ToString(nomRow["Duns No1"]);
                                        if ( (!DBNull.Value.Equals(nomRow["TransactionType"])) && PipeDunsRec==PipeDuns)
                                        {

                                            #region NonPathedRecNom

                                            V4_Nomination nomRec = new V4_Nomination();
                                            int? recQty = null;
                                            int? delQty = null;
                                            decimal? fuelPercentage = null;
                                            nomRec.TransactionType = Convert.ToString(nomRow["TransactionType"]);
                                            nomRec.NominationSubCycleIndicator = Convert.ToString(nomRow["Roll Nom"]);
                                            nomRec.ContractNumber = Convert.ToString(nomRow["Srv Req Contract"]);
                                            if (!DBNull.Value.Equals(nomRow["Fuel Percentage"])) { fuelPercentage = Convert.ToDecimal(nomRow["Fuel Percentage"]); }
                                            ...
                                            nomRec.ReceiptLocationIdentifier = Convert.ToString(nomRow["Rec Loc"]);
                                            nomRec.receiptLocationPropCode = Convert.ToString(nomRow["Rec Loc"]);

                                            nomRec.UpstreamIdentifier = Convert.ToString(nomRow["Up Identifier"]);
                                            nomRec.UpstreamContractIdentifier = Convert.ToString(nomRow["Up Contract"]);

                                            nomRec.ReceiptRank = Convert.ToString(nomRow["Rec Rank"]);
                                            nomRec.PackageId = Convert.ToString(nomRow["Package Id"]);
                                            nomRec.NominatorTrackingId = NomTrackingID(9, random);
                                            nomRec.PathType = "NPR";
                                            nomRec.UnitOfMeasure = "BZ";
                                            nomRec.QuantityTypeIndicator = "R";
                                            batch.Nominations.Add(nomRec);

                                            #endregion

                                            batch.NomSubCycle= Convert.ToString(nomRow["Roll Nom"]);
                                        }
                                        if ((!DBNull.Value.Equals(nomRow["TransactionType1"])) && PipeDunsDel == PipeDuns)
                                        {
                                            #region NonPathedDelNom

                                            V4_Nomination nomDel = new V4_Nomination();
                                            decimal? fuel = null; int? delQ = null;
                                            nomDel.TransactionType = Convert.ToString(nomRow["TransactionType1"]);
                                            nomDel.NominationSubCycleIndicator = Convert.ToString(nomRow["Roll Nom1"]);
                                            nomDel.ContractNumber = Convert.ToString(nomRow["Srv Req Contract1"]);
                                            if (!DBNull.Value.Equals(nomRow["Fuel Percentage1"])) { fuel = Convert.ToDecimal(nomRow["Fuel Percentage1"]); }

                                            nomDel.FuelPercentage = fuel;
                                            if (!DBNull.Value.Equals(nomRow["Del Qty"])) { delQ = Convert.ToInt32(nomRow["Del Qty"]); }
                                            nomDel.DelQuantity = delQ;
                                            nomDel.Quantity = 0;
                                            nomDel.DeliveryLocationIdentifer = Convert.ToString(nomRow["Del Loc"]);
                                            nomDel.DeliveryLocationPropCode = Convert.ToString(nomRow["Del Loc"]);

                                            nomDel.DownstreamIdentifier = Convert.ToString(nomRow["Down Identifier"]);
                                            nomDel.DownstreamContractIdentifier = Convert.ToString(nomRow["Down Contract"]);

                                            nomDel.DeliveryRank = Convert.ToString(nomRow["Del Rank"]);
                                            nomDel.PackageId = Convert.ToString(nomRow["Package Id1"]);
                                            nomDel.NominatorTrackingId = NomTrackingID(9, random);
                                            nomDel.PathType = "NPD";
                                            nomDel.UnitOfMeasure = "BZ";
                                            nomDel.QuantityTypeIndicator = "D";
                                            batch.Nominations.Add(nomDel);

                                            #endregion

                                            batch.NomSubCycle = Convert.ToString(nomRow["Roll Nom1"]);
                                        }
                            ...
                            }else if(notAccessedPipeduns.Count > 0 && accessedPipeduns.Count > 0)
                            {
