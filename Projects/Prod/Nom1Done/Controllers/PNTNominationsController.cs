   ...
   [Authorize]
    public class PNTNominationsController : BaseController
    {
        ...
        public ActionResult Index(Guid BatchId, int pipelineId)
        {
            ...
            else if (modelType == NomType.HyPathedNonPathed)
            {
                var CycleIndicator = _cycleIndicator.GetCycles();
                ViewBag.Cycles = CycleIndicator;
                var data =  nonPathedService.GetNonPathedNominations(pipelineId, -1, DateTime.MinValue, DateTime.Now.AddYears(1), currentIdentityValues.ShipperDuns, new Guid(currentIdentityValues.UserId),batchdetailModel.Id);
                if (data != null)
                {
                    data = UpdateCounterPartyAndLocNameInNonPathed(data);
                    batchdetailModel.ReceiptNoms = data.ReceiptNoms;
                    batchdetailModel.DeliveryNoms = data.DeliveryNoms;
                }
                else
                {
                    batchdetailModel.ReceiptNoms = new List<NonPathedRecieptNom>();
                    batchdetailModel.DeliveryNoms = new List<NonPathedDeliveryNom>();
                }
            }
        ...
        [HttpPost]
        public ActionResult NewSaveNonPathedHybrid(BatchDetailDTO Mainmodel)
        {
            ...
            Mainmodel.ShiperDuns = currentIdentityValues.ShipperDuns;

            Mainmodel.CreatedBy = currentIdentityValues.UserId;
            var pipe = pipelineService.GetPipeline(Mainmodel.PipelineId);
            Mainmodel.Duns = pipe.DUNSNo;
            Mainmodel.ShipperCompanyId = Convert.ToInt32(currentIdentityValues.CompanyId ?? "0");
            bool result = false;
            var Id = nonPathedService.SaveNewNonPathedNominations(Mainmodel);
