...
namespace Nom1Done.Controllers
{
    ...
    public class PathedNominationController : BaseController
    {
        ...
        public ActionResult Index(int? pipelineId)
        {
            ...
            if (pipe != null && (pipe.ModelTypeID == (int)NomType.PNT || pipe.ModelTypeID == (int)NomType.HyNonPathedPNT || pipe.ModelTypeID == (int)NomType.HyPathedPNT || pipe.ModelTypeID == (int)NomType.HyPathedNonPathed))
        ...
        [HttpGet]
        public ActionResult LoadNonPathedPartial(int pipelineId, string sortColumn, string sortOrder)
        {
            PathedDTO model = new PathedDTO();
            model.PipelineID = pipelineId;
            DateTime todayDate = DateTime.Now.Date;
            ...
            NonPathedDTO sortedData = new NonPathedDTO();
               var data = InonPathedService.GetNonPathedNominations(model.PipelineID, status, model.StartDate, model.EndDate, currentIdentityValues.ShipperDuns, model.showMine ? model.ShipperID.Value : Guid.Empty,Guid.Empty);
          
            if (data != null)
            {

                data = UpdateCounterPartyAndLocNameInNonPathed(data);
                sortedData = sortingbyColumn(data, sortColumn, sortOrder);
                model.ReceiptNoms = sortedData.ReceiptNoms;
                model.DeliveryNoms = sortedData.DeliveryNoms;
            }
            else {
        ...
        [HttpPost]
        public ActionResult SearchNonPathedHybrid(string startDate, string endDate, string statusId, string pipelineID, bool mine)
        {
            ...
            model.showMine = Session["showMine"] == null ? true : (bool)Session["showMine"];
            var data = InonPathedService.GetNonPathedNominations(Convert.ToInt32(pipelineID),Convert.ToInt32(statusId), Convert.ToDateTime(StartDate), Convert.ToDateTime(EndDate), currentIdentityValues.ShipperDuns, mine ? model.ShipperID.Value : Guid.Empty,Guid.Empty);
            if (data != null)
            {
                data = UpdateCounterPartyAndLocNameInNonPathed(data);
        ...
        public NonPathedDTO sortingbyColumn(NonPathedDTO dataQuery, string sortColumn, string sortOrder)
        {
            string orderDir = "DESC";
            if (!string.IsNullOrEmpty(sortOrder))
                orderDir = sortOrder.ToUpper();
            switch (sortColumn)
            {

                case "3": // StartDate
                    dataQuery.ReceiptNoms = orderDir.Equals("DESC", StringComparison.CurrentCultureIgnoreCase) ? dataQuery.ReceiptNoms.OrderByDescending(p => p.StartDateTime).ToList()
                                                                                                              : dataQuery.ReceiptNoms.OrderBy(p => p.StartDateTime).ToList();

                    dataQuery.DeliveryNoms = orderDir.Equals("DESC", StringComparison.CurrentCultureIgnoreCase) ? dataQuery.DeliveryNoms.OrderByDescending(p => p.StartDateTime).ToList()
                                                                                                             : dataQuery.DeliveryNoms.OrderBy(p => p.StartDateTime).ToList();
                    break;
                case "4": // EndDate
                    dataQuery.ReceiptNoms = orderDir.Equals("DESC", StringComparison.CurrentCultureIgnoreCase) ? dataQuery.ReceiptNoms.OrderByDescending(p => p.EndDateTime).ToList()
                                                                                                                 : dataQuery.ReceiptNoms.OrderBy(p => p.EndDateTime).ToList();

                    dataQuery.DeliveryNoms = orderDir.Equals("DESC", StringComparison.CurrentCultureIgnoreCase) ? dataQuery.DeliveryNoms.OrderByDescending(p => p.EndDateTime).ToList()
                                                                                                                   : dataQuery.DeliveryNoms.OrderBy(p => p.EndDateTime).ToList();
                    break;
                case "5": // CreateDate
                    dataQuery.ReceiptNoms = orderDir.Equals("DESC", StringComparison.CurrentCultureIgnoreCase) ? dataQuery.ReceiptNoms.OrderByDescending(p => p.CreateDateTime).ToList()
                                                                                                      : dataQuery.ReceiptNoms.OrderBy(p => p.CreateDateTime).ToList();
                    dataQuery.DeliveryNoms = orderDir.Equals("DESC", StringComparison.CurrentCultureIgnoreCase) ? dataQuery.DeliveryNoms.OrderByDescending(p => p.CreateDateTime).ToList()
                                                                                                                  : dataQuery.DeliveryNoms.OrderBy(p => p.CreateDateTime).ToList();
                    break;
                case "6": // Cycle
                    dataQuery.ReceiptNoms = orderDir.Equals("DESC", StringComparison.CurrentCultureIgnoreCase) ? dataQuery.ReceiptNoms.OrderByDescending(p => p.CycleId).ToList()
                                                                                                                     : dataQuery.ReceiptNoms.OrderBy(p => p.CycleId).ToList();
                    dataQuery.DeliveryNoms = orderDir.Equals("DESC", StringComparison.CurrentCultureIgnoreCase) ? dataQuery.DeliveryNoms.OrderByDescending(p => p.CycleId).ToList()
                                                                                                  : dataQuery.DeliveryNoms.OrderBy(p => p.CycleId).ToList();
                    break;
                case "7": //RollNom
                    dataQuery.ReceiptNoms = orderDir.Equals("DESC", StringComparison.CurrentCultureIgnoreCase) ? dataQuery.ReceiptNoms.OrderByDescending(p => p.NomSubCycle).ToList()
                                                                                                                 : dataQuery.ReceiptNoms.OrderBy(p => p.NomSubCycle).ToList();

                    dataQuery.DeliveryNoms = orderDir.Equals("DESC", StringComparison.CurrentCultureIgnoreCase) ? dataQuery.DeliveryNoms.OrderByDescending(p => p.NomSubCycle).ToList()
                                                                                                             : dataQuery.DeliveryNoms.OrderBy(p => p.NomSubCycle).ToList();
                    break;


                case "8": // TransactionType
                    dataQuery.ReceiptNoms = orderDir.Equals("DESC", StringComparison.CurrentCultureIgnoreCase) ? dataQuery.ReceiptNoms.OrderByDescending(p => p.TransactionType).ToList()
                                                                                                                 : dataQuery.ReceiptNoms.OrderBy(p => p.TransactionType).ToList();
                    dataQuery.DeliveryNoms = orderDir.Equals("DESC", StringComparison.CurrentCultureIgnoreCase) ? dataQuery.DeliveryNoms.OrderByDescending(p => p.TransactionType).ToList()
                                                                                             : dataQuery.DeliveryNoms.OrderBy(p => p.TransactionType).ToList();
                    break;

                case "9": // SrvReqContract
                    dataQuery.ReceiptNoms = orderDir.Equals("DESC", StringComparison.CurrentCultureIgnoreCase) ? dataQuery.ReceiptNoms.OrderByDescending(p => p.ServiceRequesterContractName).ToList()
                                                                                                                : dataQuery.ReceiptNoms.OrderBy(p => p.ServiceRequesterContractName).ToList();

                    dataQuery.DeliveryNoms = orderDir.Equals("DESC", StringComparison.CurrentCultureIgnoreCase) ? dataQuery.DeliveryNoms.OrderByDescending(p => p.ServiceRequesterContractName).ToList()
                                                                                                              : dataQuery.DeliveryNoms.OrderBy(p => p.ServiceRequesterContractName).ToList();

                    break;
                case "10": //RecLocProp //DelLocProp
                    dataQuery.ReceiptNoms = orderDir.Equals("DESC", StringComparison.CurrentCultureIgnoreCase) ? dataQuery.ReceiptNoms.OrderByDescending(p => p.ReceiptLocProp).ToList()
                                                                                                                : dataQuery.ReceiptNoms.OrderBy(p => p.ReceiptLocProp).ToList();

                    dataQuery.DeliveryNoms = orderDir.Equals("DESC", StringComparison.CurrentCultureIgnoreCase) ? dataQuery.DeliveryNoms.OrderByDescending(p => p.DeliveryLocProp).ToList()
                                                                                            : dataQuery.DeliveryNoms.OrderBy(p => p.DeliveryLocProp).ToList();
                    break;
                case "11": // RecLocName //DelLocName
                    dataQuery.ReceiptNoms = orderDir.Equals("DESC", StringComparison.CurrentCultureIgnoreCase) ? dataQuery.ReceiptNoms.OrderByDescending(p => p.ReceiptLocName).ToList()
                                                                                                               : dataQuery.ReceiptNoms.OrderBy(p => p.ReceiptLocName).ToList();
                    dataQuery.DeliveryNoms = orderDir.Equals("DESC", StringComparison.CurrentCultureIgnoreCase) ? dataQuery.DeliveryNoms.OrderByDescending(p => p.DeliveryLocName).ToList()
                                                                                                      : dataQuery.DeliveryNoms.OrderBy(p => p.DeliveryLocName).ToList();
                    break;
                case "12": // RecLoc //DelLoc
                    dataQuery.ReceiptNoms = orderDir.Equals("DESC", StringComparison.CurrentCultureIgnoreCase) ? dataQuery.ReceiptNoms.OrderByDescending(p => p.ReceiptLocId).ToList()
                                                                                                                : dataQuery.ReceiptNoms.OrderBy(p => p.ReceiptLocId).ToList();

                    dataQuery.DeliveryNoms = orderDir.Equals("DESC", StringComparison.CurrentCultureIgnoreCase) ? dataQuery.DeliveryNoms.OrderByDescending(p => p.DeliveryLocId).ToList()
                                                                                                                : dataQuery.DeliveryNoms.OrderBy(p => p.DeliveryLocId).ToList();
                    break;
                case "13": //UpID prop //DnProp
                    dataQuery.ReceiptNoms = orderDir.Equals("DESC", StringComparison.CurrentCultureIgnoreCase) ? dataQuery.ReceiptNoms.OrderByDescending(p => p.UpstreamProp).ToList()
                                                                                                              : dataQuery.ReceiptNoms.OrderBy(p => p.UpstreamProp).ToList();
                    dataQuery.DeliveryNoms = orderDir.Equals("DESC", StringComparison.CurrentCultureIgnoreCase) ? dataQuery.DeliveryNoms.OrderByDescending(p => p.DnstreamProp).ToList()
                                                                                                             : dataQuery.DeliveryNoms.OrderBy(p => p.DnstreamProp).ToList();
                    break;
                case "14": //UpName // DnName
                    dataQuery.ReceiptNoms = orderDir.Equals("DESC", StringComparison.CurrentCultureIgnoreCase) ? dataQuery.ReceiptNoms.OrderByDescending(p => p.UpstreamName).ToList()
                                                                                                              : dataQuery.ReceiptNoms.OrderBy(p => p.UpstreamName).ToList();
                    dataQuery.DeliveryNoms = orderDir.Equals("DESC", StringComparison.CurrentCultureIgnoreCase) ? dataQuery.DeliveryNoms.OrderByDescending(p => p.DnstreamName).ToList()
                                                                                                                : dataQuery.DeliveryNoms.OrderBy(p => p.DnstreamName).ToList();
                    break;
                case "15": //UpID //DnID 
                    dataQuery.ReceiptNoms = orderDir.Equals("DESC", StringComparison.CurrentCultureIgnoreCase) ? dataQuery.ReceiptNoms.OrderByDescending(p => p.UpstreamId).ToList()
                                                                                                             : dataQuery.ReceiptNoms.OrderBy(p => p.UpstreamId).ToList();
                    dataQuery.DeliveryNoms = orderDir.Equals("DESC", StringComparison.CurrentCultureIgnoreCase) ? dataQuery.DeliveryNoms.OrderByDescending(p => p.DnstreamId).ToList()
                                                                                                            : dataQuery.DeliveryNoms.OrderBy(p => p.DnstreamId).ToList();
                    break;
                case "16": //UpContract //DnContract
                    dataQuery.ReceiptNoms = orderDir.Equals("DESC", StringComparison.CurrentCultureIgnoreCase) ? dataQuery.ReceiptNoms.OrderByDescending(p => p.UpstreamK).ToList()
                                                                                                             : dataQuery.ReceiptNoms.OrderBy(p => p.UpstreamK).ToList();

                    dataQuery.DeliveryNoms = orderDir.Equals("DESC", StringComparison.CurrentCultureIgnoreCase) ? dataQuery.DeliveryNoms.OrderByDescending(p => p.DnstreamK).ToList()
                                                                                         : dataQuery.DeliveryNoms.OrderBy(p => p.DnstreamK).ToList();
                    break;
                case "17": //ReceiptQty //DelQty
                    dataQuery.ReceiptNoms = orderDir.Equals("DESC", StringComparison.CurrentCultureIgnoreCase) ? dataQuery.ReceiptNoms.OrderByDescending(p => p.ReceiptQty).ToList()
                                                                                                              : dataQuery.ReceiptNoms.OrderBy(p => p.ReceiptQty).ToList();
                    dataQuery.DeliveryNoms = orderDir.Equals("DESC", StringComparison.CurrentCultureIgnoreCase) ? dataQuery.DeliveryNoms.OrderByDescending(p => p.DeliveryQty).ToList()
                                                                                                            : dataQuery.DeliveryNoms.OrderBy(p => p.DeliveryQty).ToList();

                    break;
                case "18": //RecRank // DelRank 
                    dataQuery.ReceiptNoms = orderDir.Equals("DESC", StringComparison.CurrentCultureIgnoreCase) ? dataQuery.ReceiptNoms.OrderByDescending(p => p.ReceiptRank).ToList()
                                                                                                               : dataQuery.ReceiptNoms.OrderBy(p => p.ReceiptRank).ToList();

                    dataQuery.DeliveryNoms = orderDir.Equals("DESC", StringComparison.CurrentCultureIgnoreCase) ? dataQuery.DeliveryNoms.OrderByDescending(p => p.DeliveryRank).ToList()
                                                                                                               : dataQuery.DeliveryNoms.OrderBy(p => p.DeliveryRank).ToList();

                    break;
                case "19": //PackageID 
                    dataQuery.ReceiptNoms = orderDir.Equals("DESC", StringComparison.CurrentCultureIgnoreCase) ? dataQuery.ReceiptNoms.OrderByDescending(p => p.PackageId).ToList()
                                                                                                               : dataQuery.ReceiptNoms.OrderBy(p => p.PackageId).ToList();

                    dataQuery.DeliveryNoms = orderDir.Equals("DESC", StringComparison.CurrentCultureIgnoreCase) ? dataQuery.DeliveryNoms.OrderByDescending(p => p.PackageId).ToList()
                                                                                                               : dataQuery.DeliveryNoms.OrderBy(p => p.PackageId).ToList();

                    break;

                default:
                    dataQuery.ReceiptNoms = dataQuery.ReceiptNoms.OrderByDescending(p => p.CreateDateTime).ToList();
                    dataQuery.DeliveryNoms = dataQuery.DeliveryNoms.OrderByDescending(p => p.CreateDateTime).ToList();
