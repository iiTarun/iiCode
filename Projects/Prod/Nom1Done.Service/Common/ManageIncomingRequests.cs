...
namespace Nom1Done.Common
{
    public class ManageIncomingRequests : IManageIncomingRequestService
    {
                        ...
                        try
                        {
                            ...
                            if (dataSeperator != '\0' && segmentSeperator != '\0')
                            {
                                ...
                                foreach (string[] separateFile in separateSTFiles)
                                {
                                    ...
                                    if(shipperDuns=="")
                                    GenerateResponseAsync(NmqrList);
                                }
        ...
        public void GenerateResponseAsync(List<NMQRPerTransaction> NmqrList)
        {
            try
            {
                ...
                if (NmqrList.Count > 0)
                {
                    var NomRef = NmqrList.Select(a => a.ReferenceNumber).Distinct();
                    foreach(var refNum in NomRef)
                    {
                        var nomStatus = _serviceNomStatus.GetNomStatusOnReferenceNumber(refNum);
                        var batch = _serviceBatch.GetByTransactionID(nomStatus.NOM_ID);
                        var kochId = batch.Nominations.FirstOrDefault().KochId;
                        string status = string.Empty;
                        switch (nomStatus.StatusID)
                        {
                            case (int)statusBatch.Success_NMQR:
                                status = "Accepted";
                                break;
                            case (int)statusBatch.Failure_NMQR:
                                status = "Rejected";
                                break;
                            case (int)statusBatch.Failure_Gisb:
                            case (int)statusBatch.Failure_Ack:
                                status = "Fail";
                                break;
                        }
                        response.Add(new { KochId = kochId, ResponseMessage = status });
                    }
                    //response back to koch
                    Task.Run(async ()=> await SendResponseBackToShipperComp(response));
                }
            }
            catch(Exception ex)
            {
        ...
        private static async Task<bool> SendResponseBackToShipperComp(dynamic response)
        {
            DateTime start = DateTime.Now;
            bool IsDone = true;
            do
            {
                if ((DateTime.Now - start).TotalMinutes >= 3)
                {
                    break;
                }
            } while (IsDone);
