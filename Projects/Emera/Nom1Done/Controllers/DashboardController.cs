    ...
    public class DashboardController : BaseController
    {
       ...
       // [Authorize]
        public ActionResult Index()
        {
            ...
            var reqId = Request["pipelineId"] == null ? dashboardService.GetFirstPipelineByUser(id,GetCurrentCompanyID()).ID.ToString() : Request["pipelineId"];
            int pipelineID = Convert.ToInt32(reqId);// == 0 ? pipelineService.GetAllPipelineList(GetCurrentCompanyID()).OrderBy(a=>a.Name).FirstOrDefault().ID : Convert.ToInt32(reqId);  //default data for Algonquin Gas Transmission
            ViewBag.pipelineID = pipelineID;
            var pipelineDuns = pipelineService.GetDunsByPipelineID(pipelineID);
            Dashboard.RejectedNomList = dashboardService.GetRejectedNomination(pipelineID,id).ToList().OrderByDescending(a => a.FlowDate).Take(10).ToList();
