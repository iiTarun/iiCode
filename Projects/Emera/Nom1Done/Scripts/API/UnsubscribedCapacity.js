$(document).keypress(function (e) {
    if (e.which == 13) {
        if (document.getElementById("dvSearch").style.display == 'block') { SearchUnsubscribedCapacity(); }
    }
});
...
function SearchUnsubscribedCapacity() {
    ...
    if (isValid) {
        ...
        var jsonList = Post('/UnsubscribedCapacity/Search?Id=' + pipelineID, BOCapacity);
        var Source = "";
        ...
        if (jsonList != 0) {
            ...
            Source += '<th class="table-header table-row-width-5" >Loc</th>';
            Source += '<th class="table-header table-row-width-5" >Loc Purp Desc</th>';
            Source += '<th class="table-header table-row-width-5" >Loc / QTI</th>';
            Source += '<th class="table-header table-row-width-5" >Unsubscribed Capacity</th>';
            ...
            for (i = 0; i < jsonList.length; i++) {
                ...
                Source += '  <td class="font-size-12">' + jsonList[i].UnsubscribedCapacity + '</td>';
            ...
            var Message = 'No Unsubscribed Capacity data found.';
...
function LoadPaging() {
    ...
    var TotalRecords = Post('/UnsubscribedCapacity/SearchCount?Id=' + pipelineID, BOCapacity);
