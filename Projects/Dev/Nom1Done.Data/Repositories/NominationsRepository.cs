...
namespace Nom1Done.Data.Repositories
{
    public class NominationsRepository : RepositoryBase<V4_Nomination>, INominationsRepository
    {
        ...
        public IQueryable<BatchDTO> GetAllNominationBatch(string PipelineDuns, int statusID, DateTime StartDate, DateTime EndDate, Guid _providerUserKey, string shipperDuns)
        {

            DateTime SDate = StartDate.Date;
            DateTime EDate = EndDate.Date;
            List<int> statusIds = GetStatusList(statusID);
            IQueryable<BatchDTO> batchs;
            try
            {
                if (Guid.Empty == _providerUserKey) { 
                     batchs = (from a in this.DbContext.V4_Batch                            
                    ...
                    batchs = (from a in this.DbContext.V4_Batch                                
                              ...
                              && (DbFunctions.TruncateTime(a.FlowStartDate) >= SDate && DbFunctions.TruncateTime(a.FlowEndDate) <= EDate)
                              && a.ServiceRequester == shipperDuns
                              && a.CreatedBy == _providerUserKey.ToString()
                              && (statusIds.Contains(a.StatusID))
                              orderby a.CreatedDate
                              select new BatchDTO
                              {
            ...
            {
                throw new Exception("error in GetAllNominationBatch() in NominationRepo ", ex);
            }
        }
        public IEnumerable<RejectedNomModel> GetAllRejectedNoms(string pipelineDuns, string userId)
        {
            var rejectedNoms = (from a in this.DbContext.NominationStatus
                                ...
                                join c in this.DbContext.Pipeline on b.PipelineDuns equals c.DUNSNo
        ...
        public int GetPathedListTotalCount(string PipelineDuns, int Status, DateTime StartDate, DateTime EndDate, string shipperDuns, Guid loginUser)
        {
            ...
            {
                return DbContext.V4_Batch.Where(a => a.PipelineDuns == PipelineDuns
        ...
        public List<int> GetStatusList(int Status)
        {
            List<int> statusIds = new List<int>();
            switch (Status)
            {
        ...
        public List<PathedNomDetailsDTO> GetPathedListWithPaging(string PipelineDuns, int Status, DateTime StartDate, DateTime EndDate, string shipperDuns, Guid loginUser, SortingPagingInfo sortingPagingInfo)
        {
            ...
            DateTime SDate = StartDate.Date;
            DateTime EDate = EndDate.Date;
            List<int> statusIds = GetStatusList(Status);           
            ...
            if (loginUser == Guid.Empty)
            {
                dataQuery = (from B in DbContext.V4_Batch
                            ...
                            // join P in DbContext.Pipeline on B.PipelineDuns equals P.DUNSNo
                             ...
                             //from ai in ORL.Where(a => a.Identifier != null && a.PipeDuns == PipelineDuns).DefaultIfEmpty()
                             //join DL in DbContext.Location on N.DeliveryLocationIdentifer equals DL.Identifier into ODL
                             //from bi in ODL.Where(a => a.Identifier != null && a.PipeDuns == PipelineDuns).DefaultIfEmpty()
                             ...
                             where B.PipelineDuns == PipelineDuns
                            ...
                            && (statusIds.Contains(B.StatusID))
                             select new PathedNomDetailsDTO()
                             {
                                 ...
                                 RecLocation = N.ReceiptLocationName,
            ...
            List<PathedNomDetailsDTO> resultData = new List<PathedNomDetailsDTO>();
            if (skipVal == 0 && takeVal == 0)
            {
                resultData = dataQueryWithOrder.ToList();
            }           
            else if (takeVal == 0)
            {
                resultData = dataQueryWithOrder.Skip(() => skipVal).Take(1).ToList();
            }
            else {
                resultData = dataQueryWithOrder.Skip(() => skipVal).Take(() => takeVal).ToList();
            }
            ...
            foreach (var obj in resultData)
            {
                ...
                if (TransType != null)
                {
                    var ttmap = (from tt in this.DbContext.metadataTransactionType
                                 ...
                                 where ttm.PipeDuns == obj.PipelineDuns
        ...
        public NonPathedDTO GetNonPathedNominations(string PipelieDuns, int Status, DateTime StartDate, DateTime EndDate, string shipperDuns, Guid loginUser)
        {
            try
            {
                List<V4_Batch> batchList = new List<V4_Batch>();
                ...
                var resultData = (from B in DbContext.V4_Batch
                                  ...
                                  where B.PipelineDuns == PipelieDuns
                ...
                foreach (var item in resultData.Where(a => a.QuantityType == "R"))
                {
                    ...
                    recNom.PipelineDuns = item.PipelieDuns;
                ...
                foreach (var item in resultData.Where(a => a.QuantityType == "D"))
                {
                    ...
                    delNom.PipelineDuns = item.PipelieDuns;
                ...
                nonPathedDTO.PipelineDuns = PipelieDuns;
        ...
        public List<PathedNomDetailsDTO> GetPathedList(string PipelieDuns, int Status, DateTime StartDate, DateTime EndDate, string shipperDuns, Guid loginUser)
        {
            List<V4_Batch> batchList = new List<V4_Batch>();
            ...
            var resultData = DbContext.V4_Batch.Where(a => a.PipelineDuns == PipelieDuns
                                                         && a.NomTypeID == 1
                                                         && (DbFunctions.TruncateTime(a.FlowStartDate) >= SDate && DbFunctions.TruncateTime(a.FlowEndDate) <= EDate)
                                                         && a.IsActive
                                                         && a.ServiceRequester == shipperDuns
                                                         ...
                                                         && (statusIds.Contains(a.StatusID)))
                                                          .GroupJoin((from a in DbContext.V4_Nomination
                                                                      ...
                                                                      join p in this.DbContext.Pipeline on b.PipelineDuns equals p.DUNSNo
                                                                      ...
                                                                      from ai in rloter.Where(a => a.Identifier != null && a.PipeDuns == PipelieDuns).DefaultIfEmpty()

                                                                      join dl in DbContext.Location on a.DeliveryLocationIdentifer equals dl.Identifier into dloter
                                                                      from bi in dloter.Where(a => a.Identifier != null && a.PipeDuns == PipelieDuns).DefaultIfEmpty()
            ...
            foreach (var obj in resultData)
            {
                ...
                pathed.PipelineDuns = obj.BatchTable.PipelineDuns;
                ...
                if (obj.NomTable.FirstOrDefault() != null)
                {
                    ...
                    pathed.PipelineDuns = obj.NomTable.FirstOrDefault().PipelieDuns;
        ...
        public BatchDetailDTO GetNominationDetail(Guid batchId, string pipeDuns)
        {
            ...
            if (DbContext.V4_Nomination.Where(a => a.TransactionID == batchId).Count() > 0)
            {
                ...
                if (data != null && data.Count > 0)
                {
                    ...
                    // Batch. = data.FirstOrDefault().Duns;
            ...
            else
            {
                Batch = (from sNom in DbContext.V4_Batch
                        // join p in this.DbContext.Pipeline on sNom.PipelineDuns equals p.DUNSNo
        ...
        public List<SummaryDTO> GetAcceptedNomsOnDate(int month, string pipelineDuns, string UserId,string shipperCompanyDuns, bool ByLoc,bool showZero)
        {
            var sumaryNomNew = new List<SummaryDTO>();
            int daysInMonth = DateTime.DaysInMonth(DateTime.Now.Year, month);
            DateTime dtFrom = new DateTime(DateTime.Now.Year, month, 1);
        ...
        public List<SummaryDTO> GetAcceptedNomOnDate(int month, string PathType, string PipelineDuns, string UserId,string shipperCompanyDuns, bool ByLoc,bool showZero)
        {
            var sumaryNomNew = new List<SummaryDTO>();
        ...
        public List<SummaryDTO> GetNomQtyForContractPath(int month, string PipelineDuns, string UserId,string shipperCompanyDuns, bool ByLoc, bool showZero)
        {
            var sumaryNomNew = new List<SummaryDTO>();
            int daysInMonth = DateTime.DaysInMonth(DateTime.Now.Year, month);
            DateTime dtFrom = new DateTime(DateTime.Now.Year, month, 1);
    ...
    public interface INominationsRepository : IRepository<V4_Nomination>
    {
        ...
        IEnumerable<RejectedNomModel> GetAllRejectedNoms(string pipelineDuns, string userId);
        IQueryable<BatchDTO> GetAllNominationBatch(string PipelineDuns, int statusID, DateTime StartDate, DateTime EndDate, Guid _providerUserKey, string shipperDuns);
        BatchDetailDTO GetNominationDetail(Guid batchId, string pipeDuns);
        void SaveChages();
        ...
        List<PathedNomDetailsDTO> GetPathedList(string PipelieDuns, int Status, DateTime StartDate, DateTime EndDate, string shipperDuns, Guid loginUser);
        ...
        NonPathedDTO GetNonPathedNominations(string PipelieDuns, int Status, DateTime StartDate, DateTime EndDate, string shipperDuns, Guid loginUser);
        List<SummaryDTO> GetAcceptedNomsOnDate(int month, string pipelineDuns, string UserId, string shipperCompanyDuns, bool ByLoc, bool showZero);
        List<SummaryDTO> GetAcceptedNomOnDate(int month, string PathType, string PipelineDuns, string UserId, string shipperCompanyDuns, bool ByLoc, bool showZero);
        List<SummaryDTO> GetNomQtyForContractPath(int month, string PipelineDuns, string UserId, string shipperCompanyDuns, bool ByLoc, bool showZero);
    }
}
