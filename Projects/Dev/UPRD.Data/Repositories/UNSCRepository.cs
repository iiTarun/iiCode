...
namespace UPRD.Data.Repositories
{
    public class UNSCRepository : RepositoryBase<UnscPerTransaction>, IUNSCRepository
    {
        ...
        public bool SaveUnscList(List<UnscPerTransaction> unscList, int pipeId)
        {
            try
            {
                if (unscList.Count > 0)
                {
                    ...
                    {
                        var exItem = this.DbContext.UnscPerTransaction.Where(a => a.Loc == item.Loc
                         && a.PostingDateTime == item.PostingDateTime
                         && a.EffectiveGasDayTime == item.EffectiveGasDayTime
                         && a.PipelineID == pipeId);
                        if (exItem.Count() > 0)
                        {
                            this.DbContext.UnscPerTransaction.RemoveRange(exItem);
                            this.DbContext.SaveChanges();
                        }
                    }
                    this.DbContext.UnscPerTransaction.AddRange(unscList.Select(a=> { a.PipelineID = pipeId;return a; }).ToList());
                    this.DbContext.SaveChanges();
                    return true;
                }
                return false;
    ...
    public interface IUNSCRepository : IRepository<UnscPerTransaction>
    {
        ...
        bool SaveUnscList(List<UnscPerTransaction> unscList, int pipeId);
