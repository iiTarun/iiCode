...
namespace Nom1Done.Receive.Scheduler
{
    public class ReceiveWebScheduler : Ninject.Modules.NinjectModule
    {
        ...
        public static void start()
        {
            ...
            #endregion
            #region job for receive files processing
            ProcessReceiveFileTryAgain_JobCreation(_jobScheduler);
            #endregion
        }
        ...
        #region Job Creation for processing the receive files Try Again
        private static void ProcessReceiveFileTryAgain_JobCreation(IScheduler _jobScheduler)
        {
            try
            {
               // TimeAndFreqForReceiveFileProcess = _serviceSetting.GetById((int)Settings.TimeAndFreqForReceiveFileProcess).Value;
                IJobDetail EncEDIGenerationJobDetail = JobBuilder.Create<JobManagerReceiveFileProcessingTryAgain>()
                                                    .WithIdentity(string.Format("{0}", "ReceiveFileJobTryAgain"))
                                                    .Build();
                ITrigger EncEDIGenerationJobTrigger = TriggerBuilder.Create()
                                                    .WithIdentity(string.Format("{0}", "ReceiveFileJobTryAgain"))
                                                    .StartNow()
    ...
    public class JobManagerReceiveFileProcessingTryAgain : IJob
    {
        public void Execute(IJobExecutionContext context)
        {
            try
