...
namespace Nom1Done.Common
{
    public class ManageIncomingRequests : IManageIncomingRequestService
    {
            ...
            try
            {
                ...
                try
                {
                    ...
                    if (!string.IsNullOrEmpty(message))
                        decFile = DecryptFile(new StringBuilder(message), shipperDuns, _senderDuns, false);
                    else
        ...
        private string GenerateGisb(int code, bool isTest, string releaseToShipper)
        {
            ...
            if (releaseToShipper == "101069263")//Emera duns
            {
        ...
        public StringBuilder DecryptFile(StringBuilder decFile, string shipperDuns, string _senderDuns,bool fromKoch)
        {
            //Pipeline pipe = null;
            ...
            if (shipperDuns == "832703297")
            {
                passPhrase = _serviceSetting.GetById((int)Settings.KochPassphrase).Value;
                privateKey = _serviceSetting.GetById((int)Settings.KochPrivateKey).Value;
            }
            if (fromKoch)
            {
                //pgp.DecryptFile
        ...
        public string ProcessRequestKoch(HttpRequestBase request, bool isTest)
        {
            int dataset = 0;
            bool ForNom = true;
            string newGisb = "false";
            try
            {
                ...
                {
                    if (!string.IsNullOrEmpty(message))
                        decFile = DecryptFile(new StringBuilder(message), shipperDuns, _senderDuns, true);
                    else
                        GenerateAndSendErrorMail("Koch Received file is found empty", "received file is empty ", isTest, ForNom);

                }
                catch (Exception ex)
                {
                    AppLogManager("Koch Receiving File Encryption in web.", "Error", ex.Message + " InnerException:- " + ex.InnerException, ForNom);
                    return "Wrong Encryption key.";
                }
            }
            catch (Exception ex)
            {
                AppLogManager("Koch Receiving File in web.", "Error", ex.Message + " InnerException:- " + ex.InnerException, ForNom);
                GenerateAndSendErrorMail("Koch Receiving File in web.", ex.Message, isTest, ForNom);
                return newGisb;
            }
            return newGisb;
        }
