...
using RestSharp;
using Nom1Done.Service.CustomSerialization;
...
namespace Nom1Done.Service
{
    public class PNTNominationService : IPNTNominationService
    {
        ...
        public bool CopyNomination(Guid TransactionId)
        {
            try
            {
                ...
                if (batch != null)
                {
                    ...
                    newBatch.PipelineDuns = batch.PipelineDuns;
        ...
        public Guid UpdatePNTBatch(BatchDetailDTO batch)
        {
            try
            {
                var b = BatchRepository.GetByTransactionID(batch.Id);
                if ((b != null)  && (b.StatusID == (int)NomStatus.Draft) )  // only draft can update. draft=11;
                {
                    Random ran = new Random();
                    string path = Path.GetRandomFileName();
                    path = path.Replace(".", "");
            ...
            try
            {
                ...
                if (batchDetail.SupplyList != null)
                {
                   // BatchNoms = new List<V4_Nomination>();
                    foreach (var supply in batchDetail.SupplyList)
                    {
                        ...
                        Contract con = contractService.GetContractByContractNo(supply.ServiceRequestNo, batchDetail.Duns);
                        if (con == null)
                        {
                            flyCon = AddContractOnFly(supply.ServiceRequestNo, batchDetail.Duns, batchDetail.ShipperCompanyId, batchDetail.CreatedBy,string.IsNullOrEmpty(supply.FuelPercentage)?0:Convert.ToDecimal(supply.FuelPercentage));
                ...
                if (batchDetail.Contract != null)
                {
                   // BatchNoms = new List<V4_Nomination>();
                    ...
                    {
                        ContractsDTO flyCon = new ContractsDTO();
                        Contract con = contractService.GetContractByContractNo(contract.ServiceRequestNo, batchDetail.Duns);// && a.PipelineID == batchDetail.PipelineId).FirstOrDefault();
                        if (con == null)
                        {
                            flyCon = AddContractOnFly(contract.ServiceRequestNo, batchDetail.Duns, batchDetail.ShipperCompanyId, batchDetail.CreatedBy,string.IsNullOrEmpty(contract.FuelPercentage)?0:Convert.ToDecimal(contract.FuelPercentage));
                        ...
                        var ttcontract = metadataTransactionTypeRepository.GetTTUsingIdentifier(contract.TransactionType, "C", batchDetail.Duns);
                ...
                if (batchDetail.MarketList != null)
                {
                   // BatchNoms = new List<V4_Nomination>();
                    ...
                    {
                        ContractsDTO flyCon = new ContractsDTO();
                        Contract con = contractService.GetContractByContractNo(market.ServiceRequestNo,batchDetail.Duns);
                        if (con == null)
                        {
                            flyCon = AddContractOnFly(market.ServiceRequestNo, batchDetail.Duns, batchDetail.ShipperCompanyId, batchDetail.CreatedBy, string.IsNullOrEmpty(market.FuelPercentage) ? 0 : Convert.ToDecimal(market.FuelPercentage));
        ...
        public bool DirectSent(BatchDetailDTO batchDetail, bool sendToTest)
        {
            ...
            if (batchDetail.Id == Guid.Empty)
            {
                ...
                SavePNTNomination(batchDetail);
                if (ValidateNomination(batchDetail.Id, batchDetail.Duns))
                {
        ...
        public IEnumerable<ContractsDTO> GetContracts(int ShipperID, string PipelineDuns)
        {
            List<ContractsDTO> items = new List<ContractsDTO>();
            List<Contract> conList = contractRepository.GetByPipeNShipper(PipelineDuns, ShipperID).ToList();
       ...
       // }
        ...
        public IEnumerable<TransactionTypesDTO> GetTransactionsTypes(string pipeLineDuns, string Keyword, string popUpfrom)
        {
        ...
        public bool SendNominationTransaction(Guid transactionId, int ShipperCompanyID,bool sendToTest)
        {
            try
            {
                ...
                if (batch != null)
                {
                    ...
                    outbox.PipelineDuns = batch.PipelineDuns;
                    ...
                    try
                    {
                        ...
                        var pipe = pipelineService.GetPipelineByDunsNo(batch.PipelineDuns);                       
                        var nomDetail = NominationsRepository.GetAllNomsByTransactionId(batch.TransactionID);                     
                        var shipperComp = ShipperCompanyRepository.GetShipperCompanyByDuns(batch.ServiceRequester);
                        var user = shipperRepo.GetbyUserId(batch.CreatedBy);
                        DashNominationStatusDTO nomDas = new DashNominationStatusDTO();
                        nomDas.Environment = environment;                       
                        nomDas.PipeDuns = pipe != null ? pipe.DUNSNo : string.Empty;
                        nomDas.PipelineName = pipe != null ? pipe.Name : string.Empty;
                        nomDas.ShipperCompany = shipperComp != null ? shipperComp.Name : string.Empty;
                        nomDas.ShipperDUNS = shipperComp!=null?shipperComp.DUNS:string.Empty;
                        nomDas.StatusId = 2;
                        nomDas.NomStatus =  nomDas.StatusId == 0 ? "GISB Unprocessed" : (nomDas.StatusId == 5 ? "Submitted" : (nomDas.StatusId <= 4  ? "In-Process" : "Not defined"));
                        nomDas.SubmittedDate = DateTime.Now;
                        nomDas.TransactionId = batch.TransactionID;
                        nomDas.UserName = user != null ? user.FirstName + " " + user.LastName : string.Empty;
                        if (nomDetail.Count > 0)
                        {
                            foreach (var Nomination in nomDetail)
                            nomDas.NomTrackingId = (Nomination.NominatorTrackingId !=null ? Nomination.NominatorTrackingId:"");
                        }                                        
                        string apiBaseUrl = ConfigurationManager.AppSettings.Get("BaseUrlOfUprdApi");
                        RestClient client = new RestClient(apiBaseUrl + "/api/DashNominationStatus/");
                        RestRequest request = new RestRequest("PostDashNominationStatu", Method.POST);
                        request.JsonSerializer = NewtonsoftJsonSerializer.Default;
                        request.AddJsonBody(nomDas);
                        IRestResponse<bool> res = client.Execute<bool>(request);
                    }
                    catch(Exception ex)
                    {
        ...
        public IEnumerable<BatchDTO> GetBatches(string Keyword, string PipelineDuns, int statusID, DateTime StartDate, DateTime EndDate, int PageNo, int PageSize, Guid _providerUserKey, string shipperDuns)
        {
                IEnumerable<BatchDTO> batchList = new List<BatchDTO>();
           
                batchList = NominationsRepository.GetAllNominationBatch(PipelineDuns,statusID,StartDate,EndDate,_providerUserKey,shipperDuns).ToList();
                ...
                //    {
                //        if (statusID == -1)
                //            batchList = batchList.Where(a => a.IsActive == true && a.pipeDUNSNo == PipelineDuns && ((a.DateBeg.Date) >= StartDate.Date && (a.DateEnd.Date) <= EndDate.AddDays(1).Date) && a.ServiceRequester == shipperDuns );
                //        else
                //        {
                //            batchList = batchList.Where(a => a.IsActive == true && a.pipeDUNSNo == PipelineDuns && ((a.DateBeg.Date) >= StartDate.Date && (a.DateEnd.Date) <= EndDate.AddDays(1).Date) && a.ServiceRequester == shipperDuns ).ToList();
                ...
                //    {
                //        if (statusID == -1)
                //            batchList = batchList.Where(a => a.IsActive == true && a.pipeDUNSNo == PipelineDuns && ((a.DateBeg.Date) >= StartDate.Date && (a.DateEnd.Date) <= EndDate.AddDays(1).Date) && a.ServiceRequester == shipperDuns && a.CreatedBy == _providerUserKey.ToString());
                //        else
                //        {
                //            batchList = batchList.Where(a => a.IsActive == true && a.pipeDUNSNo == PipelineDuns && ((a.DateBeg.Date) >= StartDate.Date && (a.DateEnd.Date) <= EndDate.AddDays(1).Date) && a.ServiceRequester == shipperDuns && a.CreatedBy == _providerUserKey.ToString()).ToList();
                //            batchList = GetBatchAccordingToStatus(batchList.ToList(), statusID);
                //        }
                //    }
                //}
        ...
        public bool ValidateNomination(Guid transactioId, string pipeDuns)
        {
            bool reqFields = true;
            bool pathComplete = true;
            BatchDetailDTO batchDetail = NominationsRepository.GetNominationDetail(transactioId, pipeDuns);//GetNominationDetailByBatchID(transactioId);
        ...
        public BatchDetailDTO GetNomDetail(Guid batchId, string pipeDuns)
        {
            var query = NominationsRepository.GetNominationDetail(batchId, pipeDuns);
            return query;
        }
