    ...
    public class PathedNominationController : BaseController
    {
        ...
        public ActionResult Index(string pipelineDuns)
        {
            ...
            if (pipe != null && (pipe.ModelTypeID == (int)NomType.PNT || pipe.ModelTypeID == (int)NomType.HyNonPathedPNT || pipe.ModelTypeID == (int)NomType.HyPathedPNT))
                ...
                return RedirectToAction("Index", "NonPathed", new { pipelineDuns = pipelineDuns });
            ...
            int pathedListTotalCount = _pathedNominationService.GetPathedListTotalCount(pipe.DUNSNo, status, model.StartDate, model.EndDate, model.DunsNo,model.showMine? model.ShipperID.Value: Guid.Empty);
            ...
            List<PathedNomDetailsDTO> nomlist = _pathedNominationService.GetPathedListWithPaging(pipe.DUNSNo, status, model.StartDate, model.EndDate, model.DunsNo,model.showMine ? model.ShipperID.Value: Guid.Empty, model.SortingPagingInfo);
            ...
            model.PipelineDetails = pipe.Name;// + " (" + pipe.DUNSNo + ")";
            ...
            model.PipelineNomType = GetPathTypeByPipelineType(pipe.ModelTypeID);
        ...
        {
            var duns = model.PipelineDuns;//_pipelineService.GetDunsByPipelineID(model.PipelineID);
            int tt = 0;
        ...
        public ActionResult Index(PathedDTO model, string Search)
        {
            ...
            else
            {
                ...
                var pipelineDuns = model.PipelineDuns;
                model.DunsNo = currentIdentityValues.ShipperDuns;

                model.ShipperID = Guid.Parse(currentIdentityValues.UserId);
                int pathedListTotalCount = _pathedNominationService.GetPathedListTotalCount(model.PipelineDuns, Status, StartDate, EndDate, model.DunsNo, model.showMine?model.ShipperID.Value:Guid.Empty);
                ...
                var PathedNomsList = _pathedNominationService.GetPathedListWithPaging(model.PipelineDuns, Status, StartDate, EndDate, model.DunsNo, model.showMine ? model.ShipperID.Value : Guid.Empty, model.SortingPagingInfo);
                ...
                var pipes = GetPipelines(); var pipe = pipes.Where(a => a.DUNSNo == pipelineDuns).FirstOrDefault();
                model.PipelineNomType = GetPathTypeByPipelineType(pipe.ModelTypeID);               
                model.PathedNomsList = model.PathedNomsList.OrderByDescending(a => a.CreatedDate).ToList();
        ...
        public PartialViewResult GetLocationPopUpForSpecialHybrid(string TransTypeMapId, string clickedRow, string popUpFor, string PipelineDuns)
        {
        ...
        public PartialViewResult NotimationsPartials(string partial, string clickedRow, string popUpFor, string PipelineDuns)
        {
            ...
            else if (partial == "TransactionType")
            {
                ...
                model.TransactionTypes = _pntNominationService.GetTransactionsTypes(PipelineDuns, "", model.PopUpFor).ToList();
            ...
            else if (partial == "Contract")
            {
                ...
                //TODO:
                model.Contracts = _pntNominationService.GetContracts( companyId, PipelineDuns).ToList();
                model.PipelineDuns = PipelineDuns;
        ...
        public PartialViewResult AddPathedNomRow(int RowCount, string pipelineDuns)
        {
            PathedDTO model = new PathedDTO();
            ...
            var pipe = pipeLines.Where(a => a.DUNSNo == pipelineDuns).FirstOrDefault();
            var modelType= GetPathTypeByPipelineType(pipe.ModelTypeID );
        ...
        public PartialViewResult CopyRow(List<PathedNomDetailsDTO> PathedRecordToCopy, string pipelineDuns)
        {
            PathedDTO model = new PathedDTO();
            model.PipelineDuns = pipelineDuns;
        ...
        public ActionResult GetDataForSortingPaging(PathedFilters pathedFilters)
        {
           ...
           // int pageSize = Convert.ToInt32(Request.Form.GetValues("length")[0]);
            ...
            model.SortingPagingInfo = info;
            model.PipelineDuns = pathedFilters.PipelineDuns;
            model.StartDate = pathedFilters.StartDate;
            model.EndDate = pathedFilters.EndDate;
            model.StatusId = pathedFilters.StatusId;

            List<PathedNomDetailsDTO> nomlist = _pathedNominationService.GetPathedListWithPaging(model.PipelineDuns, model.StatusId, model.StartDate, model.EndDate, currentIdentityValues.ShipperDuns, model.showMine ? Guid.Parse(currentIdentityValues.UserId) : Guid.Empty , model.SortingPagingInfo);
            model.PathedNomsList = UpdatePathedCounterPartyAndLoactionName(nomlist);

            model.PathedNomsList= SortedByLocNCounterPartyNames(model.PathedNomsList, model.SortingPagingInfo);
        ...
        public List<PathedNomDetailsDTO> SortedByLocNCounterPartyNames(List<PathedNomDetailsDTO> dataQuery, SortingPagingInfo sortingPagingInfo)
        {
            string orderDir = sortingPagingInfo.SortDirection;
            switch (sortingPagingInfo.SortField)
            {
                case "10": // Receipt Loc name
                    dataQuery = orderDir.Equals("DESC", StringComparison.CurrentCultureIgnoreCase) ? dataQuery.OrderByDescending(p => p.RecLocation).ToList()
                                                                                                                  : dataQuery.OrderBy(p => p.RecLocation).ToList();
                    break;
                case "13": // Up stream Name
                    dataQuery = orderDir.Equals("DESC", StringComparison.CurrentCultureIgnoreCase) ? dataQuery.OrderByDescending(p => p.UpName).ToList()
                                                                                                                   : dataQuery.OrderBy(p => p.UpName).ToList();
                    break;
                case "19": // DelLoc
                    dataQuery = orderDir.Equals("DESC", StringComparison.CurrentCultureIgnoreCase) ? dataQuery.OrderByDescending(p => p.DelLoc).ToList()
                                                                                                                   : dataQuery.OrderBy(p => p.DelLoc).ToList();
                    break;
                case "22": // Down Stream name
                    dataQuery = orderDir.Equals("DESC", StringComparison.CurrentCultureIgnoreCase) ? dataQuery.OrderByDescending(p => p.DownName).ToList()
                                                                                                                   : dataQuery.OrderBy(p => p.DownName).ToList();
                    break;
            }
            return dataQuery;
        }
        ...
        [HttpPost]
        public JsonResult GetNonPathedTypeByTT(int tt, string ttDesc, string pipelineDuns)
        {
            int resultType = 0;
            //var duns = _pipelineService.GetDunsByPipelineID(pipelineID);
            resultType = NonPathedTypeVariance.GetNomModelType(pipelineDuns, tt, ttDesc);
            //  var propertiesList= PathedNonpathedHybridProperties.GetPropertiesByTypes(resultType);
            return Json(resultType);
        }

        public ActionResult GetLocationForPopUp(string pipelineDuns, string PopUpFor, bool IsSpecialDelCase)
        {
        ...
        public ActionResult GetCounterParties(string pipelineDuns)
        {
        ...
        [HttpGet]
        public ActionResult LoadPartialPathed(string pipelineDuns)
        {
            SortingPagingInfo info = new SortingPagingInfo();
            info.PageSize = pageSize;
            info.CurrentPageIndex = 0;
            ...
            model.DunsNo = currentIdentityValues.ShipperDuns;
            model.PipelineDuns = pipelineDuns;
            int pathedListTotalCount = _pathedNominationService.GetPathedListTotalCount(model.PipelineDuns, status, model.StartDate, model.EndDate, model.DunsNo,model.showMine? model.ShipperID.Value: Guid.Empty);

            info.PageCount = (pathedListTotalCount % info.PageSize) != 0 ? (pathedListTotalCount / info.PageSize) + 1 : (pathedListTotalCount / info.PageSize);

            model.SortingPagingInfo = info;
            List<PathedNomDetailsDTO> nomlist = _pathedNominationService.GetPathedListWithPaging(model.PipelineDuns, status, model.StartDate, model.EndDate, model.DunsNo, model.showMine ? model.ShipperID.Value: Guid.Empty, model.SortingPagingInfo);
            model.PathedNomsList = UpdatePathedCounterPartyAndLoactionName(nomlist);
            // var pipelineData = _pathedNominationService.GetPipeline(pipelineId);
            var pipeLines = GetPipelines();
            var pipe = pipeLines!=null ? pipeLines.Where(a => a.DUNSNo == pipelineDuns).FirstOrDefault() : new PipelineDTO();
            model.PipelineDetails = pipe.Name + "(" +  pipelineDuns + ")";
            model.PipelineDuns = pipelineDuns;
            model.StartDate = new DateTime(DateTime.Now.Year, DateTime.Now.Month, 1);
            model.EndDate = model.StartDate.AddMonths(2);
            var CycleIndicator = _cycleIndicator.GetCycles();
            ...
            NomType modelType = GetPathTypeByPipelineType(pipe.ModelTypeID);
                //_pipelineService.GetPathTypeByPipelineDuns(_pipelineService.GetDunsByPipelineID(pipelineId));
        ...
        [HttpGet]
        public ActionResult LoadNonPathedPartial(string pipelineDuns)
        {
            PathedDTO model = new PathedDTO();
            model.PipelineDuns = pipelineDuns;
            DateTime todayDate = DateTime.Now.Date;
            DateTime mindate = todayDate.AddYears(-1);
            DateTime maxDate = todayDate.AddYears(1);
            ShipperReturnByIdentity currentIdentityValues = GetValueFromIdentity();
            int status = -1;
            var data = InonPathedService.GetNonPathedNominations(model.PipelineDuns, status, mindate, maxDate, currentIdentityValues.ShipperDuns, new Guid(currentIdentityValues.UserId));
        ...
        public ActionResult SaveNonPathedHybrid(PathedDTO Mainmodel)
        {
            ...
            model.PipelineDuns = Mainmodel.PipelineDuns;
            ...
            return RedirectToAction("Index", new { pipelineDuns = Mainmodel.PipelineDuns });
        }
        [HttpPost]
