    ...
    public class SendPartFunctions
    {
        ...
        private void RaiseAndSendUprdRequest(Pipeline pipe, bool isOacy, bool isUnsc, bool isSwnt, bool isTest,bool sendManu)
        {
            try
            {
                ...
                outbox.PipelineDuns = pipe.DUNSNo;
                outbox.CompanyID = 2;
                ...
                string shipperDuns = ConfigurationManager.AppSettings["ReleaseToShipper"];
                var pipelineEdiSetting = _modalFactory.Parse(_servicePipelineEdiSetting.GetPipelineSetting(pipe.DUNSNo, (int)DataSet.Upload_of_Requests_for_Download_of_Posted_Datasets, shipperDuns));
                if (pipelineEdiSetting != null)
                {
                    ...
                    if (!string.IsNullOrEmpty(ediFile.ToString()))
                    {
                        ...
                        SaveXmlToDB(job.TransactionId, pipe.DUNSNo, null, ediFile.ToString(), null, null);
            ...
            try
            {
                ...
                if (pendingJob != null)
                {
                    ...
                    try
                    {
                        ...
                        MaintainLogNominationEDIJobs(pendingJob, batch.PipelineDuns, workFlow);
                        #endregion
                ...
                string shipperDuns = ConfigurationManager.AppSettings["ReleaseToShipper"];
                PipelineEDISetting pipelineEDISetting = _servicePipelineEdiSetting.GetPipelineSettingForManuallySend((int)DataSet.Upload_of_Requests_for_Download_of_Posted_Datasets, shipperDuns);
                if (pipelineEDISetting != null)
                {
        ...
        private void MaintainLogNominationEDIJobs(TaskMgrJob pendingJob, string pipelineDuns, JobWorkflow workFlow)
        {
            #region Nomination generator
            StringBuilder GeneratedNomEDI = new StringBuilder(GenerateEDINomination(pendingJob.TransactionId, pipelineDuns, workFlow.WorkflowId));
            if (!String.IsNullOrEmpty(GeneratedNomEDI.ToString()))
            {
                SaveXmlToDB(pendingJob.TransactionId, pipelineDuns, null, GeneratedNomEDI.ToString(), null, null);
        ...
        public string GenerateEDINomination(string TransactionId, string pipelineDuns, Int64 workFlowId)
        {
            try
            {
               ...
               // Pipeline pipe = _servicePipeline.GetById(pipeID);
                ...
                if (batch != null && batch.NomTypeID == (int)NomType.PNT)
                {
                    ...
                    var pipelineEdiSetting = _modalFactory.Parse(_servicePipelineEdiSetting.GetPipelineSetting(pipelineDuns, (int)DataSet.Nomination_PNT, shipperCompDuns));
                ...
                else if (batch != null && batch.NomTypeID == (int)NomType.Pathed)
                {
                    ...
                    int shipperCompId = _serviceShipper.GetbyUserId(batch.CreatedBy).ShipperCompanyID;
                    string shipperCompDuns = _serviceShipperCompany.GetById(shipperCompId).DUNS;
                    var pipelineEdiSetting = _modalFactory.Parse(_servicePipelineEdiSetting.GetPipelineSetting(pipelineDuns, (int)DataSet.Nomination, shipperCompDuns));
                    if (NomList.Count() > 0 && pipelineEdiSetting != null)
                    {
                        NMST_GN ediGen = new NMST_GN();
        ...
        private void StartEncryptingEDI(TaskMgrJob job, Int64 workFlowId)
        {
            try
            {
                ...
                if (EDIFile != null)
                {
                    ...
                    metadataPipelineEncKeyInfo keyInfo = _servicemetadataPipelineEncKeyInfo.GetByPipelineDuns(EDIFile.PipeDuns);
                    ...
                    SaveXmlToDB(job.TransactionId, EDIFile.PipeDuns, null, null, encEDIFile, null);
        ...
        private void SendMailOfSendingFile(string transactionId, bool GISBReceived)
        {
            try
            {
                ...
                if (newFile != null)
                {
                    Pipeline pipeline = _servicePipeline.GetPipelineByDuns(newFile.PipelineDuns);
        ...
        private HttpWebResponse CollectingDataForSendingFile(string transactionId)
        {
            try
            {
                ...
                {
                    Pipeline pipe = _servicePipeline.GetPipelineByDuns(outbox.PipelineDuns);
                    if (pipe == null)
                    ...
                    else
                    {
                        ...
                        else
                        {
                            ...
                            else
                            {
                                ...
                                return SendFileToTSP(outbox.ID, sendingFilePacket, pipe.DUNSNo, outbox.IsTest, shipper.Name, transactionId);
                            }
                        }
                    }
                }
        ...
        private HttpWebResponse SendFileToTSP(int outboxID, Dictionary<string, object> sendingFilePacket, string pipeDuns, bool isTest, string shipperName, string transactionId)
        {
            //Console.WriteLine("****************************************************************************");
            //Console.WriteLine("\r\nSendzFileToTSP");
            ...
            {
                TradingPartnerWorksheet tpw = _serviceTPW.GetByPipelineDuns(pipeDuns);
                if (tpw == null)
                    throw new Exception("TPW is null for pipeline " + pipeDuns);
                else
                {
            ...
            SaveXmlToDB(transactionId, pipeDuns, null, null, null, postURL);
            ...
            try
            {
                ...
                else
                {
                    ...
                    if (batch != null)
                    {
                        ...
                        #region API call for update nom statue
                        string apiBaseUrl = ConfigurationManager.AppSettings.Get("BaseUrlOfUprdApi");
                        var client = new RestClient(apiBaseUrl + "/api/DashNominationStatus/");
                        var request = new RestRequest("PutDashNominationStatu", Method.PUT);
                        request.AddQueryParameter("TransactionId", batch.TransactionID.ToString());
                        request.AddQueryParameter("statusId", batch.StatusID.ToString());
        ...
        #endregion
        #region  Logging
        private void SaveXmlToDB(string transactionId, string pipeduns, string Xmltemplate, string EdiFile, string EncEDIFile,string sendingUrl)
        {
            ...
            else
            {
                ...
                xml.PipeDuns = string.IsNullOrEmpty(pipeduns) ? xml.PipeDuns : pipeduns;
                if (Xmltemplate != null)
                    xml.XmlData = Xmltemplate;
                if (EdiFile != null)
                    xml.EDIData = EdiFile;
